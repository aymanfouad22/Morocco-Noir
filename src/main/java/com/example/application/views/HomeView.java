...
import com.example.application.views.City1;
import com.example.application.views.Game1;
import com.example.application.views.Scenario1;
...
public class HomeView extends Composite<VerticalLayout> {
    ...
    private Label countdownLabel;  // Label to display the countdown
    private ScheduledExecutorService executorService;  // Executor service for countdown
    private int countdownTime = 300; 
    ArrayList<Button> gameButton = new ArrayList<>();
...
//            }
    ...
    public HomeView() {
            ...
            String secret = dotenv.get("DB_PASSWORD");

            conversation = new OpenAIConversation(secret, "gpt-4o");
            ...
            countdownLabel = new Label("Time remaining: 05:00");
            countdownLabel.getStyle().set("font-size", "24px");  // Set font size
            countdownLabel.getStyle().set("font-weight", "bold");  // Set font weight to bold
            countdownLabel.getStyle().set("text-align", "center");  // Center the text
            countdownLabel.getStyle().set("width", "100%");  // Make the label take full width for centering
            countdownLabel.getStyle().set("display", "flex");  // Use flexbox for centering
            countdownLabel.getStyle().set("justify-content", "center");  // Center horizontally
            ...
            if(countdownTime == 0 ){
                replyText.setText("Game Over ! You couldn't find the criminal in the given time. ");
                gameOver();
            }
            ...
            gameButton.add(marketButton);
            gameButton.add(cafeButton);
            gameButton.add(cityButton1);
            gameButton.add(cityButton2);
            gameButton.add(cityButton3);
            gameButton.add(cityButton4);
            gameButton.add(cityButton5);
            gameButton.add(questionButton);
            gameButton.add(addEvidenceButton);
            gameButton.add(submitGuessButton); 
    ...
    private void startCountdown() {
        ...
        executorService.scheduleAtFixedRate(() -> {
            if (countdownTime > 0) {
                ...
                getUI().ifPresent(ui -> ui.access(() -> {
                    replyText.setText("Time's up! The game is over.");
                    gameOver();
                }));
            }
        ...
        private void updateCountdownLabel() {
            ...
            // Ensure UI update is done in the UI thread
            getUI().ifPresent(ui -> ui.access(() -> {
 ...
 private void gameOver(){
    ...
    button.getStyle().set("color", "darkgrey"); // Optional: Change text color to make it more visible
